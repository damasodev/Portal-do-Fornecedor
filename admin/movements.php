...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($s) {
        $supply_id = $s['id'];
        $new_stock = $type === 'entrada' ? $s['stock'] + $quantity : $s['stock'] - $quantity;
        $pdo->prepare("UPDATE supplies SET stock = ? WHERE id = ?")->execute([$new_stock, $supply_id]);

        $pdo->prepare("INSERT INTO movements (supply_id, type, quantity, user_id, printer_id, created_at)
            VALUES (?, ?, ?, ?, ?, NOW())")
            ->execute([$supply_id, $type, $quantity, $_SESSION['user']['id'], $printer_id]);
    }
